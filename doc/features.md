...
| **Review mode** | 2026-02-17 | `swarm review` — Provide feedback on a previous run for agents to fix/improve. Loads previous run's context (spec, tasks, design, stream results), skips completed phases, re-runs implement with previous output + user feedback. Supports `--run <runId>` to target a specific run. Checkpoint/resume and auto-retry work the same as run mode |
