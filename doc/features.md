...
| **Review mode** | 2026-02-17 | `swarm review` — Provide feedback on a previous run for agents to fix/improve. Loads previous run's context, uses AI triage to determine which streams are affected by feedback — only affected streams re-run, unchanged streams are skipped. Engineers see previous output + feedback. Supports `--run <runId>`. Checkpoint/resume and auto-retry work the same as run mode |
...
| **TUI header info** | 2026-02-19 | TUI dashboard header shows CLI version, active model, and current working directory (smartly shortened) |
