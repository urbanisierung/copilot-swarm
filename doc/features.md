...
| **Fleet mode (multi-repo)** | 2026-02-27 | `swarm fleet` — Orchestrate features across multiple repositories. Meta-orchestrator analyzes all repos in parallel, strategist agent produces cross-repo plan with shared contracts and execution waves, repos run via independent `swarm task` processes per wave. Cross-repo reviewer validates consistency. Supports `fleet.config.yaml` or `--repos` CLI args. Checkpoint/resume at fleet level |
...
| **TUI per-agent timing** | 2026-02-28 | TUI "Active Agents" column shows elapsed time per agent (e.g. `01:23`), updates live |
| **Agent usage stats** | 2026-02-28 | Every agent invocation recorded to `.swarm/stats.json` with call count, model, and elapsed time. Accumulates across runs |
| **Stats command** | 2026-02-28 | `swarm stats` — View aggregated agent invocation statistics: call counts, total/average time, models used, token counts |
| **Central backup store** | 2026-02-28 | All `.swarm/` artifacts auto-synced to `~/.config/copilot-swarm/backups/` on every checkpoint save and session finish. `swarm backup` for manual full sync, `swarm restore` to recover from central store |
