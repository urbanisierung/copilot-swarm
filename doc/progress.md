...
- **Pre-flight prompt truncation** — `callIsolated()` and `callIsolatedWithInstructions()` now estimate token count (system message + prompt) before sending. If the total exceeds 90% of the model's context limit (default 136K), the prompt is automatically reduced to fit. Uses a two-tier strategy: (1) **Smart truncation** for small overages (<5%) — keeps 60% from start (task context) + 30% from end (specific instructions), removes only the middle bulk. (2) **AI summarization** for larger overages — the fast model summarizes the excess content, preserving all critical technical details (file paths, code, requirements, decisions) instead of losing them. Falls back to smart truncation if summarization fails. Supports chunked summarization for extremely large prompts.
