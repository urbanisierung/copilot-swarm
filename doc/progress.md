...
### Added
- **Compare mode** (`swarm compare`) — Compare multiple PRs/branches side-by-side. Accepts 2+ repo paths as positional arguments (labeled A, B, C, ...). Detects changed files via `git diff`, filters noise directories (`.github/`, `node_modules/`, `dist/`, lock files, etc.), runs parallel Diff Analyst agents per repo, optionally evaluates against a requirements file (`-f`), then a Comparative Reviewer synthesizes a comprehensive Markdown report. Options: positional paths, `--left`/`--right` (legacy compat), `--base` (default: main), `-o` (output path). Three agents: `diff-analyst` (fast model, parallel), `requirements-evaluator` (primary model, conditional), `comparative-reviewer` (primary model). Token limits handled via `callIsolated` with `fitToTokenBudget` pre-flight and `truncateForSend` safety net. TUI dashboard integration. 21 tests.
...
- **Layered error recovery** — Four-layer error recovery system: (1) Smart retries using error classification — transient errors get exponential backoff, permanent errors fail immediately instead of wasting retries. (2) Deterministic context reduction for token-limit errors — progressive truncation of repo context, spec, and optional components. (3) AI recovery agent — when deterministic reduction is insufficient, a fast-model agent analyzes the prompt component breakdown and decides what to trim/drop. (4) Actionable fail-fast messages — auth errors suggest `gh auth login`, rate limits suggest waiting, context-length errors show exact overage. New `ContextLengthError` typed error carries `{ promptTokens, limit, overage }`. Verify phase now caps implementation context at 32K tokens. `errors.ts` module with `shouldRetry()`, `estimateCallTokens()`, `reducePrompt()`, and `applyRecoveryActions()`.
- 173 tests across 9 test files (22 new error recovery tests).
...
- **Pre-flight prompt protection on all code paths** — Token budget enforcement now applies to **every** AI call, not just `callIsolated`/`callIsolatedWithInstructions`. The `send()` method — used by multi-turn conversations (plan mode PM clarification, brainstorm, etc.) — now runs a sync pre-flight check that applies smart truncation when the prompt would exceed the token budget. This catches direct `send()` callers that previously bypassed pre-flight entirely. Three key parameters: (1) `CHARS_PER_TOKEN = 3` — conservative token estimation for code-heavy content (was chars/4 which under-counted by ~30%). (2) `SDK_OVERHEAD_TOKENS = 15,000` — accounts for invisible tool definitions and SDK boilerplate. (3) `PROMPT_BUDGET_RATIO = 0.9`. Combined effective budget = `limit × 0.9 − 15K`. When calling `smartTruncate`, target is adjusted for consistent allocation: `tokens × 3/4`. Two-tier strategy: (1) **AI summarization** (via `fitToTokenBudget` in `callIsolated`) for quality reduction. (2) **Sync smart truncation** (via `truncateForSend` in `send()`) as universal safety net.
