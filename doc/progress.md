...
- **Deep analysis for complex directories** — Directories with more than 10 source files (configurable via `PREPARE_DEEP_THRESHOLD`) automatically use a two-phase analysis. Phase 1: a scout agent (fast model) identifies logical groups/topics within the directory. Phase 2: parallel agents (primary model) produce per-group instruction files with deep architectural analysis — covering group purpose, data flow, design patterns, invariants, error handling, extension points, and common pitfalls. Falls back to simple single-file analysis if scouting fails. File naming uses `<dir>-<group>.instructions.md` pattern.
